...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const prices = menProducts.map(product => product.price);
...
function filterProducts() {
    // Сначала отфильтровываем только мужские товары
    return products.filter(product => {
        // Показываем только товары категории "мужчинам"
        ...
        if (!activeFilters.subcategories.includes('all')) {
            ...
            const titleToSubcategory = {
                'Классическая рубашка': 'shirts',
                'Базовая футболка': 'tshirts',
                'Джинсы прямого кроя': 'jeans',
                'Кожаная куртка': 'outerwear'
            };
